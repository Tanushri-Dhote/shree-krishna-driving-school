...
export default function AdmissionDetailView({ admissions, onBack }: AdmissionDetailViewProps) {
  ...
  const pendingCount = admissions.filter((a) => a.status?.toLowerCase() === "pending").length;
  ...
  const kpiCards = [
    { label: "Total Admissions", count: totalAdmissions, icon: "📋", color: "orange", gradient: "from-orange-50 to-orange-100" },
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500 mt-2 font-medium">Manage and track all driving school admission applications</p>
        </div>
        <button
          onClick={onBack}
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div className="relative flex-1">
            <input
              ...
              className="w-full rounded-xl border border-slate-300 bg-white px-5 py-3 pl-12 text-sm outline-none transition-all focus:border-orange-500 focus:ring-2 focus:ring-orange-200"
            />
            <svg
          ...
          <div className="flex gap-3 flex-wrap">
            <select
              ...
              className="px-5 py-3 rounded-xl border border-slate-300 bg-white text-slate-700 font-semibold text-sm outline-none focus:border-orange-500 focus:ring-2 focus:ring-orange-200"
            >
            ...
            <button className="px-6 py-3 rounded-xl bg-orange-500 hover:bg-orange-600 text-white font-semibold text-sm transition-all shadow-md hover:shadow-lg flex items-center gap-2">
              📥 Export
            </button>
          </div>
        </div>
          ...
          <table className="min-w-full">
            <thead>
              <tr className="bg-gradient-to-r from-slate-50 to-white border-b border-slate-200">
                <th className="px-6 py-4 text-left text-sm font-bold text-slate-700">Admission No</th>
                ...
                paginatedData.map((item, idx) => {
                  ...
                  return (
                    <tr key={idx} className="hover:bg-orange-50 transition-colors">
                      <td className="px-6 py-4 font-bold text-slate-900 text-sm">{item.admissionNo}</td>
                      ...
                      <td className="px-6 py-4 text-center">
                        <button className="text-orange-500 hover:text-orange-700 font-semibold text-sm transition">👁️ View</button>
                      </td>
                    </tr>
                  );
                })
              ...
              return (
                <button
                  ...
                  className={`px-3 py-2 rounded-lg font-semibold transition ${
                    currentPage === pageNum ? "bg-orange-500 text-white" : "border border-slate-300 hover:bg-slate-100"
                  }`}
                >
                  {pageNum}
                </button>
