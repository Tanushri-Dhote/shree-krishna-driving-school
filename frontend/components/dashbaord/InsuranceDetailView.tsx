...
export default function InsuranceDetailView({ insurances, onBack }: InsuranceDetailViewProps) {
  ...
  const pendingCount = insurances.filter((a) => a.status?.toLowerCase() === "pending").length;
  const approvedCount = insurances.filter((a) => a.status?.toLowerCase() === "approved").length;
  const rejectedCount = insurances.filter((a) => a.status?.toLowerCase() === "rejected").length;

  const filteredData = useMemo(() => {
    return insurances.filter((item) => {
      const matchesSearch =
        item.fullName?.toLowerCase().includes(searchTerm.toLowerCase()) ||
        item.insuranceNo?.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  const kpiCards = [
    { label: "Total Insurance Applications", count: totalInsurances, icon: "🛡️", color: "emerald", gradient: "from-emerald-50 to-emerald-100" },
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500 mt-2 font-medium">Manage and track all vehicle insurance applications</p>
        </div>
        <button
          onClick={onBack}
      ...
      <div className="bg-gradient-to-r from-slate-50 via-white to-slate-50 border border-slate-200 rounded-3xl p-6 shadow-md">
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div className="relative flex-1">
            <input
              type="text"
              placeholder="Search by name, insurance no, vehicle no, mobile..."
              value={searchTerm}
              onChange={(e) => {
                setSearchTerm(e.target.value);
                setCurrentPage(1);
              }}
              className="w-full rounded-xl border border-slate-300 bg-white px-5 py-3 pl-12 text-sm outline-none transition-all focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200"
            />
            <svg
          ...
          <div className="flex gap-3 flex-wrap">
            <select
              ...
              className="px-5 py-3 rounded-xl border border-slate-300 bg-white text-slate-700 font-semibold text-sm outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200"
            >
            ...
            <button className="px-6 py-3 rounded-xl bg-emerald-500 hover:bg-emerald-600 text-white font-semibold text-sm transition-all shadow-md hover:shadow-lg flex items-center gap-2">
              📥 Export
            </button>
          </div>
        </div>
          ...
          <table className="min-w-full">
            <thead>
              <tr className="bg-gradient-to-r from-slate-50 to-white border-b border-slate-200">
                <th className="px-6 py-4 text-left text-sm font-bold text-slate-700">Insurance No</th>
                <th className="px-6 py-4 text-left text-sm font-bold text-slate-700">Candidate Name</th>
                <th className="px-6 py-4 text-left text-sm font-bold text-slate-700">Vehicle No</th>
                ...
                paginatedData.map((item, idx) => {
                  ...
                  return (
                    <tr key={idx} className="hover:bg-emerald-50 transition-colors">
                      <td className="px-6 py-4 font-bold text-slate-900 text-sm">{item.insuranceNo}</td>
                      <td className="px-6 py-4 font-semibold text-slate-800 text-sm">{item.fullName}</td>
                      <td className="px-6 py-4 text-slate-700 text-sm">{item.vehicleNo}</td>
                      ...
                      <td className="px-6 py-4 text-center">
                        <button className="text-emerald-500 hover:text-emerald-700 font-semibold text-sm transition">👁️ View</button>
                      </td>
                    </tr>
                  );
                })
              ...
              return (
                <button
                  ...
                  className={`px-3 py-2 rounded-lg font-semibold transition ${
                    currentPage === pageNum ? "bg-emerald-500 text-white" : "border border-slate-300 hover:bg-slate-100"
                  }`}
                >
                  {pageNum}
                </button>
