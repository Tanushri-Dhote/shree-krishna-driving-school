...
export default function InsuranceView() {
  ...
  async function updateStatus(id: number, status: InsuranceStatus) {
    try {
      ...
      // Update state if selected is currently open
        ...
        <div className="border-b border-slate-200 p-6">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              ...
              <p className="text-slate-500 mt-1">Review and manage client insurance applications.</p>
            </div>
              ...
              <table className="min-w-full text-left text-sm">
                <thead>
                  <tr className="text-slate-600 bg-slate-50 border-b border-slate-100">
                    <th className="px-4 py-3.5 font-semibold">Insurance No</th>
                  ...
                  {filteredRows.map((r) => (
                    <tr key={r.id} className="align-middle hover:bg-slate-50 transition-colors">
                      <td className="px-4 py-4 whitespace-nowrap">
                        <div className="font-bold text-slate-900">{r.insuranceNo}</div>
                      ...
                      <td className="px-4 py-4 whitespace-nowrap">
                        <div className="text-slate-900 font-semibold bg-slate-100 border border-slate-200 px-2 py-0.5 rounded text-xs inline-block">
                          {r.vehicleNo.toUpperCase()}
                        </div>
                      </td>
                      <td className="px-4 py-4">
                      ...
                      <td className="px-4 py-4 whitespace-nowrap text-right">
                        <div className="flex justify-end items-center gap-2">
                          ...
                          <select
                            value={r.status}
                            onChange={(e) => updateStatus(r.id, e.target.value as InsuranceStatus)}
            ...
            <div className="p-6 border-b border-slate-200 flex justify-between items-center bg-slate-50">
              <div>
                ...
                <h3 className="text-xl font-bold text-slate-900">
                  {selected.fullName} — {selected.insuranceNo}
                </h3>
              </div>
              <button
                type="button"
            ...
            <div className="p-6 overflow-y-auto flex-1">
              <div className="grid md:grid-cols-3 gap-6 mb-6 text-sm">
                ...
                <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200/60">
                  ...
                  <p className="text-slate-800 font-semibold mt-0.5 uppercase">{selected.vehicleNo}</p>
                </div>
              </div>

              <h4 className="text-base font-bold text-slate-800 mb-3 flex items-center gap-2">
                📄 Compulsory Documents (PAN, Aadhaar and RC Both Sides)
              </h4>

              <div className="grid sm:grid-cols-2 gap-6">

                {/* PAN Photo */}
                <div className="border border-slate-200 rounded-2xl p-4 bg-slate-50 text-center flex flex-col justify-between">
                  <div>
                    <h5 className="font-semibold text-slate-800 text-sm">PAN Card</h5>
                    <p className="text-[11px] text-slate-400 mb-2">Compulsory PAN registration photo</p>
                  </div>
                  <div className="aspect-[4/3] bg-white rounded-xl overflow-hidden flex items-center justify-center border border-slate-200 relative group">
                  ...
                  <a
                    ...
                    className="mt-3 block text-center px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-800 rounded-xl text-xs font-semibold transition-all"
                  >
                    Download File
                  </a>
                </div>
                ...
                <div className="border border-slate-200 rounded-2xl p-4 bg-slate-50 text-center flex flex-col justify-between">
                  <div>
                    ...
                    <p className="text-[11px] text-slate-400 mb-2">Compulsory Aadhaar card photo</p>
                  </div>
                  <div className="aspect-[4/3] bg-white rounded-xl overflow-hidden flex items-center justify-center border border-slate-200 relative group">
                ...
                <div className="border border-slate-200 rounded-2xl p-4 bg-slate-50 text-center flex flex-col justify-between">
                  <div>
                    <h5 className="font-semibold text-slate-800 text-sm">RC Book (Front Side)</h5>
                    <p className="text-[11px] text-slate-400 mb-2">Compulsory RC book front side photo</p>
                  </div>
                  <div className="aspect-[4/3] bg-white rounded-xl overflow-hidden flex items-center justify-center border border-slate-200 relative group">
                    {selected.rcFrontPhoto.startsWith("data:") ? (
                      // eslint-disable-next-line @next/next/no-img-element
                      <img src={selected.rcFrontPhoto} alt="RC Front Side" className="object-contain w-full h-full p-1" />
                    ) : (
                      <p className="text-slate-400 text-xs">No preview available</p>
                    )}
                  </div>
                  <a
                    href={selected.rcFrontPhoto}
                    download={`${selected.fullName}_RC_Front.png`}
                    className="mt-3 block text-center px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-800 rounded-xl text-xs font-semibold transition-all"
                  >
                    Download File
                  </a>
                ...
                <div className="border border-slate-200 rounded-2xl p-4 bg-slate-50 text-center flex flex-col justify-between">
                  <div>
                    <h5 className="font-semibold text-slate-800 text-sm">RC Book (Back Side)</h5>
                    <p className="text-[11px] text-slate-400 mb-2">Compulsory RC book back side photo</p>
                  </div>
                  <div className="aspect-[4/3] bg-white rounded-xl overflow-hidden flex items-center justify-center border border-slate-200 relative group">
                    {selected.rcBackPhoto.startsWith("data:") ? (
                      // eslint-disable-next-line @next/next/no-img-element
                      <img src={selected.rcBackPhoto} alt="RC Back Side" className="object-contain w-full h-full p-1" />
                    ) : (
                      <p className="text-slate-400 text-xs">No preview available</p>
                    )}
                  </div>
                  <a
                    href={selected.rcBackPhoto}
                    download={`${selected.fullName}_RC_Back.png`}
                    className="mt-3 block text-center px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-800 rounded-xl text-xs font-semibold transition-all"
                  >
                    Download File
                  </a>
