...
export default function InsuranceView() {
  ...
  async function load() {
    setLoading(true);
    try {
      const res = await fetch(`${backendBaseUrl}/api/insurances`);
      const payload = await res.json().catch(() => null);
      if (!res.ok) {
        toast.error(payload?.message || "Failed to fetch Insurance submissions.");
        return;
      }
      setRows((payload?.data as InsuranceRow[]) || []);
    } catch (e: any) {
      toast.error(e?.message || "Network error while fetching Insurance submissions.");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function updateStatus(id: number, status: InsuranceStatus) {
    setUpdatingId(id);
    try {
      const res = await fetch(`${backendBaseUrl}/api/insurances/${id}/status`, {
        method: "PATCH",
        headers: {
      ...
      toast.error(e?.message || "Network error while updating status.");
    } finally {
      setUpdatingId(null);
    }
  }
  ...
  const filteredRows = useMemo(() => {
    return rows.filter((r) => {
      ...
      const matchesSearch =
        ...
        r.insuranceNo.toLowerCase().includes(q);
            ...
            <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
              <div>
                <h2 className="text-2xl font-bold text-slate-900">
                  🛡️ Insurance Submissions
                </h2>
                <p className="mt-1 text-slate-500">Review and manage client insurance applications.</p>
              </div>
                ...
                <table className="min-w-full text-left text-sm">
                  <thead>
                    <tr className="bg-slate-100">
                      ...
                      <th className="px-6 py-4 text-left text-[13px] font-bold text-slate-700">Date</th>
                    ...
                    {filteredRows.map((item) => (
                      <tr key={item.id} className="transition hover:bg-orange-50 text-[14px]">
                        ...
                        <td className="px-6 py-4">
                          <div>
                            <p className="font-semibold text-slate-800 text-[11px]">{item.fullName}</p>
                          </div>
                        </td>

                        <td className="px-6 py-4 text-[11px] text-slate-700">{item.mobileNo}</td>

                        <td className="px-6 py-4 text-[11px] text-slate-600">{item.email}</td>
                ...
                <div className="border-b border-slate-200 bg-gradient-to-r from-slate-50 via-white to-slate-50 px-6 py-5">
                  <div className="flex items-start justify-between">
                    <div>
                      ...
                      <p className="mt-1 text-sm text-slate-500">Candidate Information & Uploaded Documents</p>
                    </div>
                    <button
                      type="button"
                    ...
                    <table className="min-w-full">
                      <tbody className="divide-y divide-slate-100">
                        <tr className="hover:bg-orange-50 transition">
                          ...
                          <td className="px-6 py-4 text-[13px] font-semibold text-slate-800">{selected.fullName}</td>
                        </tr>
                        <tr className="hover:bg-orange-50 transition">
                          <th className="bg-slate-50 px-6 py-4 text-left text-[11px] font-bold uppercase tracking-wide text-slate-600">
                            Mobile Number
                          </th>
                          <td className="px-6 py-4 text-[13px] text-slate-700">{selected.mobileNo}</td>
                        </tr>
                        <tr className="hover:bg-orange-50 transition">
                          <th className="bg-slate-50 px-6 py-4 text-left text-[11px] font-bold uppercase tracking-wide text-slate-600">
                            Email Address
                        ...
                        <tr className="hover:bg-orange-50 transition">
                          ...
                          <td className="px-6 py-4 text-[13px] font-semibold text-slate-800">{selected.insuranceNo}</td>
                        </tr>
                        <tr className="hover:bg-orange-50 transition">
                          <th className="bg-slate-50 px-6 py-4 text-left text-[11px] font-bold uppercase tracking-wide text-slate-600">
                            Created Date
                          </th>
                          <td className="px-6 py-4 text-[13px] text-slate-700">{new Date(selected.createdAt).toLocaleDateString("en-IN")}</td>
                        </tr>
                            ...
                            <div className="flex items-center justify-between">
                              <div className="flex items-center gap-4">
                                {selected.panPhoto?.startsWith("data:") ? (
                                  ...
                                  />
                                ) : resolveImageSrc(selected.panPhoto) ? (
                                  // eslint-disable-next-line @next/next/no-img-element
                                  <img
                                    src={resolveImageSrc(selected.panPhoto)}
                                    alt="PAN Card"
                                    className="h-24 w-32 rounded-lg border object-cover"
                                  />
                                ) : (
                                  <div className="flex h-24 w-32 items-center justify-center rounded-lg border border-dashed text-xs text-slate-400">No Image</div>
                                )}
                                <p className="text-[13px] font-medium text-slate-700">PAN Card</p>
                              </div>
                              ...
                              {selected.panPhoto ? (
                                <Button
                                  ...
                                  onClick={() => {
                                    ...
                                    const href = resolveImageSrc(selected.panPhoto) || selected.panPhoto;
                                    link.href = href;
                            ...
                            <div className="flex items-center justify-between">
                              <div className="flex items-center gap-4">
                                {selected.aadhaarPhoto?.startsWith("data:") ? (
                                  ...
                                  <img
                                    ...
                                    className="h-24 w-32 rounded-lg border object-cover"
                                  />
                                ) : resolveImageSrc(selected.aadhaarPhoto) ? (
                                  // eslint-disable-next-line @next/next/no-img-element
                                  <img
                                    src={resolveImageSrc(selected.aadhaarPhoto)}
                                    alt="Aadhaar Card"
                                    className="h-24 w-32 rounded-lg border object-cover"
                                  />
                                ) : (
                                ...
                                <p className="text-[13px] font-medium text-slate-700">Aadhaar Card</p>
                              </div>
                              ...
                              {selected.aadhaarPhoto ? (
                                <Button
                                  ...
                                  onClick={() => {
                                    const link = document.createElement("a");
                                    link.href = resolveImageSrc(selected.aadhaarPhoto) || selected.aadhaarPhoto;
                        ...
                        <tr className="transition hover:bg-orange-50">
                          ...
                          <td className="px-6 py-5">
                            <div className="flex items-center justify-between">
                              <div className="flex items-center gap-4">
                                {selected.rcFrontPhoto?.startsWith("data:") ? (
                                  // eslint-disable-next-line @next/next/no-img-element
                                  <img
                                    src={selected.rcFrontPhoto}
                                    alt="RC Front"
                                    className="h-24 w-32 rounded-lg border object-cover"
                                  />
                                ) : resolveImageSrc(selected.rcFrontPhoto) ? (
                                  // eslint-disable-next-line @next/next/no-img-element
                                  <img
                                    src={resolveImageSrc(selected.rcFrontPhoto)}
                                    alt="RC Front"
                                    className="h-24 w-32 rounded-lg border object-cover"
                                  />
                                ) : (
                                ...
                                <p className="text-[13px] font-medium text-slate-700">RC Front</p>
                              </div>

                              {selected.rcFrontPhoto ? (
                                <Button
                                  variant="outline"
                                  size="sm"
                                  onClick={() => {
                                    ...
                                    link.href =
                                      resolveImageSrc(selected.rcFrontPhoto) ||
                                      selected.rcFrontPhoto;
                                    link.download = `${selected.fullName}_RC_Front.png`;
                                    document.body.appendChild(link);
                                    link.click();
                                    document.body.removeChild(link);
                                  }}
                        ...
                        <tr className="transition hover:bg-orange-50">
                          ...
                          <td className="px-6 py-5">
                            <div className="flex items-center justify-between">
                              <div className="flex items-center gap-4">
                                {selected.rcBackPhoto?.startsWith("data:") ? (
                                  // eslint-disable-next-line @next/next/no-img-element
                                  <img
                                    src={selected.rcBackPhoto}
                                    alt="RC Back"
                                    className="h-24 w-32 rounded-lg border object-cover"
                                  />
                                ) : resolveImageSrc(selected.rcBackPhoto) ? (
                                  // eslint-disable-next-line @next/next/no-img-element
                                  <img
                                    src={resolveImageSrc(selected.rcBackPhoto)}
                                    alt="RC Back"
                                    className="h-24 w-32 rounded-lg border object-cover"
                                  />
                                ) : (
                                ...
                                <p className="text-[13px] font-medium text-slate-700">RC Back</p>
                              </div>

                              {selected.rcBackPhoto ? (
                                <Button
                                  variant="outline"
                                  size="sm"
                                  onClick={() => {
                                    ...
                                    link.href =
                                      resolveImageSrc(selected.rcBackPhoto) ||
                                      selected.rcBackPhoto;
                                    link.download = `${selected.fullName}_RC_Back.png`;
                                    document.body.appendChild(link);
                                    link.click();
                                    document.body.removeChild(link);
                                  }}
