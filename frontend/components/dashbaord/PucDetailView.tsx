...
export default function PucDetailView({ pucs, onBack }: PucDetailViewProps) {
  ...
  const pendingCount = pucs.filter((a) => a.status?.toLowerCase() === "pending").length;
  const approvedCount = pucs.filter((a) => a.status?.toLowerCase() === "approved").length;
  const rejectedCount = pucs.filter((a) => a.status?.toLowerCase() === "rejected").length;

  const filteredData = useMemo(() => {
    return pucs.filter((item) => {
      const matchesSearch =
        item.fullName?.toLowerCase().includes(searchTerm.toLowerCase()) ||
        item.pucNo?.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  }, [pucs, searchTerm, statusFilter]);

  const totalPages = Math.ceil(filteredData.length / itemsPerPage);
  const paginatedData = filteredData.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage);

  const kpiCards = [
    { label: "Total PUC Applications", count: totalPucs, icon: "📄", color: "blue", gradient: "from-blue-50 to-blue-100" },
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500 mt-2 font-medium">Manage and track all vehicle PUC certification applications</p>
        </div>
        <button
          onClick={onBack}
      ...
      <div className="bg-gradient-to-r from-slate-50 via-white to-slate-50 border border-slate-200 rounded-3xl p-6 shadow-md">
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div className="relative flex-1">
            <input
              type="text"
              placeholder="Search by name, PUC no, vehicle no, mobile..."
              value={searchTerm}
              onChange={(e) => {
                setSearchTerm(e.target.value);
                setCurrentPage(1);
              }}
              className="w-full rounded-xl border border-slate-300 bg-white px-5 py-3 pl-12 text-sm outline-none transition-all focus:border-blue-500 focus:ring-2 focus:ring-blue-200"
            />
            <svg
          ...
          <div className="flex gap-3 flex-wrap">
            <select
              ...
              className="px-5 py-3 rounded-xl border border-slate-300 bg-white text-slate-700 font-semibold text-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-200"
            >
            ...
            <button className="px-6 py-3 rounded-xl bg-blue-500 hover:bg-blue-600 text-white font-semibold text-sm transition-all shadow-md hover:shadow-lg flex items-center gap-2">
              📥 Export
            </button>
          </div>
        </div>
          ...
          <table className="min-w-full">
            <thead>
              <tr className="bg-gradient-to-r from-slate-50 to-white border-b border-slate-200">
                <th className="px-6 py-4 text-left text-sm font-bold text-slate-700">PUC No</th>
                ...
                paginatedData.map((item, idx) => {
                  ...
                  return (
                    <tr key={idx} className="hover:bg-blue-50 transition-colors">
                      <td className="px-6 py-4 font-bold text-slate-900 text-sm">{item.pucNo}</td>
                      ...
                      <td className="px-6 py-4 text-center">
                        <button className="text-blue-500 hover:text-blue-700 font-semibold text-sm transition">👁️ View</button>
                      </td>
                    </tr>
                  );
                })
              ...
              return (
                <button
                  ...
                  className={`px-3 py-2 rounded-lg font-semibold transition ${
                    currentPage === pageNum ? "bg-blue-500 text-white" : "border border-slate-300 hover:bg-slate-100"
                  }`}
                >
                  {pageNum}
                </button>
